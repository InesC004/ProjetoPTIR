  ...
  try {
    ...
    if (!nome || !nif || !email || !password || !numero_carta) {
      return res.status(400).json({
        success: false,
        message:
          "Nome, NIF, email, password e número de carta são obrigatórios.",
      });
    }
    ...
    if (!validarNIF(nif)) {
      return res
        .status(400)
        .json({ success: false, message: "NIF inválido. Deve ter 9 dígitos." });
    }
    ...
    if (codigo_postal && !validarCodigoPostal(codigo_postal)) {
      return res.status(400).json({
        success: false,
        message: "Código postal inválido. Formato: 1000-200.",
      });
    }
    ...
    const existing = await Motorista.findOne({
      $or: [{ nif }, { email }, { numero_carta }],
    });
    if (existing)
      return res
        .status(409)
        .json({ success: false, message: "NIF, email ou carta já registado." });
...
// login motorista — agora com NIF + password
exports.login = async (req, res) => {
  try {
    ...
    const match = await bcrypt.compare(access_password, motorista.password);
    if (!match)
      return res
        .status(401)
        .json({ success: false, message: "Credenciais inválidas." });
...
// atualizar motorista (só gestores)
exports.update = async (req, res) => {
  try {
    ...
    // Verifica duplicados excluindo o motorista atual
    if (email) {
      const existeEmail = await Motorista.findOne({ email, _id: { $ne: id } });
      if (existeEmail)
        return res
          .status(409)
    ...
    if (nif) {
      const existeNif = await Motorista.findOne({ nif, _id: { $ne: id } });
      if (existeNif)
        return res
          .status(409)
          .json({ success: false, message: "Este NIF já está em uso." });
    }
    if (numero_carta) {
      const existeCarta = await Motorista.findOne({
        numero_carta,
        _id: { $ne: id },
      });
      if (existeCarta)
        return res.status(409).json({
          success: false,
          message: "Este número de carta já está em uso.",
        });
    }
    ...
    if (numero_carta) updateData.numero_carta = numero_carta;

    const motorista = await Motorista.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    }).select("-password");
    if (!motorista)
...
exports.delete = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Motorista apagado com sucesso." });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Erro no servidor." });
  }
