...
exports.responderMotorista = async (req, res) => {
  try {
    ...
    if (pedido.cliente_id.toString() !== req.user.id) {
      return res.status(403).json({ success: false, message: 'Sem permissão. pedidos' })
    }
...
exports.getById = async (req, res) => {
  try {
    ...
    if (pedido.estado === 'aceite' && pedido.motorista_id) {
      ...
      const distancia = haversine(motoristaLat, motoristaLng, pedido.origem_lat, pedido.origem_lng)
      const tempoChegada = distancia * 4 // 4 minutos por km

      const distanciaViagem = haversine(pedido.origem_lat, pedido.origem_lng, pedido.destino_lat, pedido.destino_lng)
...
exports.listarDisponiveis = async (req, res) => {
  try {
    ...
    if (!turno) {
      return res.status(400).json({ success: false, message: 'Não tens nenhum turno ativo.' })
    }
...
exports.aceitar = async (req, res) => {
  try {
    ...
    if (pedido.estado !== 'pendente') {
      return res.status(400).json({ success: false, message: 'Pedido já não está disponível.' })
    }
    ...
    res.status(200).json({ success: true, message: 'Pedido aceite com sucesso.', pedido })
...
exports.cancelarAceitacao = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Aceitação cancelada, pedido voltou a pendente.', pedido })
